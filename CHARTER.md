...
- **Cloud backend** (planned): a second Terragrunt backend module targeting a managed
  ...
  requires no fork to run there. Localhost stays the default. A first attempt (Oracle
  Cloud Always Free + k3s) was built and then reverted — see
  [ADR-0028](docs/decisions/adr-0028-oracle-cloud-backend-rejected.md), which supersedes
  ADR-0027. (ADR-0026)
