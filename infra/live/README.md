...
| `oracle/` | Built, **partially verified against a real account (2026-07-15)** | [ADR-0027](../../docs/decisions/adr-0027-first-cloud-backend-oracle-always-free-k3s.md) / [RFC #377](https://github.com/tooming/k8s-anywhere/issues/377) / [issue #406](https://github.com/tooming/k8s-anywhere/issues/406). Oracle Cloud Always Free (Ampere A1) running k3s. Verified end-to-end against a real tenancy: `scripts/tfstate-oracle-bootstrap.sh` (off-cluster Garage tfstate backend), `terragrunt init` against that backend's real S3 API, and the `cluster/` unit's VCN/subnet/security-list/internet-gateway layer all apply cleanly. Found and fixed three real bugs only a live run could surface (missing `docker` group grant in the tfstate cloud-init, a VCN `dns_label` over OCI's 15-char limit, and `vm-standard-e2-1-micro-count` quota being AD-specific rather than uniform). **Not yet verified**: the k3s compute instance itself — Oracle's Ampere A1 Always Free host capacity was exhausted across all 3 ADs in `eu-frankfurt-1` at verification time (a transient, external constraint, confirmed via `500 Out of host capacity` on every AD), so `docs/done/` and this row will be updated again once a `terragrunt apply` completes an actual instance launch. |
