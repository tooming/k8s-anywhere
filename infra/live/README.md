...
Every backend directory has exactly two Terragrunt units, mirroring `local/`:
...
(A third `gitlab/` unit, later a `forgejo/` unit, configured a self-hosted git source
ArgoCD read from — both removed entirely 2026-09-07, no replacement; ArgoCD now syncs
directly from this repo's public GitHub remote on every backend, so there is nothing
left for a third unit to configure.)

**Only `cluster/` is backend-specific.** `argocd/` never changes per backend — it
depends on `cluster/`'s outputs and is otherwise identical to `local/`'s unit verbatim.
This works because of a narrow, already-in-place output contract:
...
- `kube_context` (string) — a kubectl context name that, **after `terraform apply`
  ...
  `KUBECONFIG` points) to a working connection to the cluster. `argocd/` generates its
  `helm`/`kubernetes` providers from exactly this — see its `generate "provider"`
  block, which reads `dependency.cluster.outputs.kube_context` and
  `pathexpand("~/.kube/config")` unconditionally.
- `api_endpoint` (string) — the Kubernetes API server URL. Informational; not currently
  consumed by `argocd/`, but part of the contract so tooling built against one backend
...
2. Add `infra/live/<backend>/{cluster,argocd}/terragrunt.hcl`, copying `local/`'s
   `argocd/` unit unchanged and pointing `cluster/`'s `source` at the new module.
3. Add a `root.hcl` (or extend the shared one) for the backend's Terraform state
   backend — `local/`'s local-file state backend (ADR-0007) is itself
