...
STEP 2 — Take the lab's pulse. Run these in order, capturing output:
  ...
  - Hit the UIs: `curl -s -o /dev/null -w "%{http_code}" <gateway-url>/<host>` for each always-on route in README.md's Endpoints table (ArgoCD, Vault, s3manager, …). Non-200 = broken route. (There is no Grafana any more — the observability stack was removed entirely 2026-09-06, ADR-0041, no replacement. There is no RabbitMQ, moto, ACK, or KRO either — all removed entirely with no replacement.)
