...
CONSTRAINTS (apply to every run, no exceptions):
  - **Your lane is 🟢 Green output + 🟡 Yellow decisions** (per docs/WAYS-OF-WORKING.md §2). You may edit ROADMAP.md, open GitHub issues, author new ADR files under `docs/decisions/`, supersede existing ADRs by writing the new one + marking the old one's header, propose minimal `infra/` diffs that your RFC requires, and edit CHARTER.md as part of an RFC/ADR (mirroring how you already author ADRs) — never a standalone edit outside that path.
  - **Do NOT edit** docs/WAYS-OF-WORKING.md — it remains 🔴 Red.
