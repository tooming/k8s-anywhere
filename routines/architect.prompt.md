...
Next, directly check upstream for releases in the past 7 days for each ADR'd component. Use `gh release list --repo <owner>/<repo> --limit 5` for each:
  ...
  - Kargo: `akuity/kargo`
  (Grafana, Longhorn, TiDB, Istio, RabbitMQ, and Valkey were all removed from the
  lab entirely 2026-09-06 with no replacement — ADR-0041, ADR-0013,
  ADR-0031/ADR-0032, ADR-0012, ADR-0009, ADR-0018 — no longer tracked here.)
