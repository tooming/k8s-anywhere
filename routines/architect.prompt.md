...
STEP 2 — Close out OPEN ADR audits FIRST (no audit outlives one cycle). Run `gh issue list --label adr-audit --state open`. Skip any that another agent holds `in-progress` or an open PR references (unless the claim is stale) and claim each one you take, per docs/WAYS-OF-WORKING.md §3 "Claim before you work" — its terminal outcome below (issue closed, or the `Closes #<n>` PR open) ends the claim. A held audit is handled by its holder and is exempt from the MUST below. Each `adr-audit` issue is a question you OWN, and it MUST reach a terminal decision in this run — it may never be re-deferred to "the next cycle with more signal". That re-defer is exactly what stranded issue #157 (filed, recommended "revisit", then left open with no mechanism to ever close it). For every open audit, choose exactly ONE terminal outcome and act on it in THIS run:
...
STEP 3 — Find 🟡 work. Scan ROADMAP.md for every 🟡 item. For each one, check whether a linked RFC GitHub issue already exists (look for an issue number such as `(RFC #NN)` in the item text, or an open issue whose title matches). Skip any 🟡 item that already has an open RFC issue (whether or not it carries `in-progress`; an RFC you merely file in STEP 5 is never labeled `in-progress` — that label is for whoever implements or resolves it).
...
STEP 8b — Self-review the `arch/*` PR, then self-merge (PR path only). There is no separate reviewer routine — you are also the first-pass reviewer, and per WAYS-OF-WORKING.md §0.1 you also merge, including PRs that edit an ADR or CHARTER.md — this adversarial pass is the only second voice your own decisions get before they land. Re-read your own diff (`gh pr diff <num>`) and audit it against the three review checks: (1) **Gate integrity** — no `make ci` check weakened, skipped, or stubbed; (2) **ADR compliance** — no silent contradiction of any Decision in docs/decisions/ (if you contradict one, the superseding ADR must be IN this PR); (3) **Fabricated content (ADR-0004)** — every upstream release cited points at a real tag you fetched in STEP 1, not an invented version. THEN do the adversarial design review on your own decisions:
  ...
  If a check fails, FIX it on the branch (re-run `make ci`, push) and re-audit; if it genuinely cannot be fixed this run, say so prominently in a PR comment, @-mention the maintainer, and leave the PR **open, unmerged**. Otherwise, once the audit and the adversarial design review above both pass, confirm required checks are green and conversations resolved, and **merge**: `gh pr merge <num> --squash --delete-branch`. (Do NOT use `gh pr review` — GitHub rejects reviews on a PR authored by the same token.) [The `[self-review]` PR-comment-and-label step this used to end with was dropped 2026-09-23 — see docs/WAYS-OF-WORKING.md §4 for the reasoning; the audit and design review above still run, they're just no longer posted as a comment or marked with a label.]
