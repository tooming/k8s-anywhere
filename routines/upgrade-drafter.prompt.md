...
STEP 7 — Never end empty-handed. If NO upgrade was available (everything is at latest stable), do NOT open a churn PR and do NOT fabricate one. Open (or refresh, if a matching one is already open — search `gh pr list --state open --search "in:title [Action needed]"` first) a PR on branch `upgrade/action-needed-<slug>` whose only content is a new `docs/backlog/YYYY-MM-DD-action-needed-<slug>.md` file, titled `[Action needed] <one-line summary>` (never the word "idle" — `scripts/idle-issue-guard-check.sh` unconditionally blocks any GitHub issue/comment carrying it; see docs/done/2026-07-19-action-needed-pr-fallback.md), noting "as of YYYY-MM-DD the stack is current". Run the normal self-review + self-merge contract on it (STEP 6b above, same as any other PR this routine opens).
