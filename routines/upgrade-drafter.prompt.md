...
STEP 6b — Self-review the PR (PR path only). There is no separate reviewer routine — you are also the first-pass reviewer. Re-read your own diff with adversarial eyes (`gh pr diff <num>`) and audit it against the four review checks: (1) **Gate integrity** — no `make ci` check weakened, skipped, or stubbed to make the bump pass; (2) **ADR compliance** — the bump doesn't cross a version pinned by an ADR or reintroduce a rejected technology; (3) **Tier discipline** — the diff is a same-source version bump ONLY (no new chart repo/registry, no major bump, no CI/Makefile/`infra/` touch); (4) **Fabricated content (ADR-0004)** — the changelog/release links in the PR body are real upstream URLs you actually fetched, not invented. If a check fails, FIX it on the branch (re-run `make ci`, push) and re-audit; if unfixable this run, abandon per STEP 5 rules. Then post the verdict as a PR comment (`gh pr comment <num>`) starting with the literal marker line `[self-review]`, the four ✅/❌ lines (`Gate integrity` / `ADR compliance` / `Tier discipline` / `Fabricated content`), a one-line verdict, and a note on anything caught and fixed. (Do NOT use `gh pr review` — GitHub rejects reviews on a PR authored by the same token.) Finally: `gh label create self-reviewed --color 5319E7 --description "First-pass review posted by the producing routine" 2>/dev/null || true` and `gh pr edit <num> --add-label self-reviewed`.
