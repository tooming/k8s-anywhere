...
STEP 1 — Orient. Run `git fetch origin && git checkout main && git pull --ff-only`. Then read: CHARTER.md (north-star); docs/WAYS-OF-WORKING.md (ownership map in §7); the ADRs in docs/decisions/. Read just enough of the repo to recognize each domain's path roots: `gitops/network/`, `gitops/apps/`, `infra/` + `gitops/bootstrap/` + `gitops/platform/`, `gitops/argocd/` + `gitops/cert-manager/` + `gitops/governance/`. (`gitops/observability/` no longer exists — the observability stack was removed entirely 2026-09-06, ADR-0041, no replacement. `gitops/ack/`, `gitops/kro/`, and `gitops/moto/` no longer exist either — that cloud-control-plane demo trio was removed entirely 2026-09-07, ADR-0038, no replacement. `gitops/vault/` and `gitops/secrets/` no longer exist either — Vault and External Secrets Operator were removed entirely 2026-09-07, ADR-0042, no replacement. `gitops/storage/` (Garage + s3manager, removed 2026-09-07) and `gitops/data/` (RabbitMQ + Valkey, removed 2026-09-06) no longer exist either, no replacement — see WAYS-OF-WORKING.md §7's ownership map for the current path-to-domain mapping.)

STEP 2 — Ensure the label palette exists. Run once per execution (idempotent — `|| true` swallows the "already exists" error):
  - Domain (mirrors WAYS-OF-WORKING.md §7): `domain:bootstrap`, `domain:network`, `domain:platform-core`, `domain:cloud-cp`, `domain:apps`, `domain:routines`, `domain:docs`. Use a neutral color, e.g. `BFDADC`. (`domain:observability`, `domain:secrets`, and `domain:storage` are retired, not recreated — the directories they labeled (`gitops/observability/`, `gitops/vault/` + `gitops/secrets/`, `gitops/storage/` + `gitops/data/`) were all removed entirely, no replacement — ADR-0041 (2026-09-06), ADR-0042 (2026-09-07), and the Garage/s3manager (2026-09-07) + RabbitMQ/Valkey (2026-09-06) removals respectively; an existing issue still carrying one of these labels is left as-is, not relabeled.)
  ...
  Example: `gh label create domain:platform-core --color BFDADC --description "Owner: gitops/argocd/, gitops/cert-manager/, gitops/governance/" 2>/dev/null || true`.
