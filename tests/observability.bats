...
# Garage). These assert structural integrity — dashboard file
...
# --- Lab — Envoy Gateway (Ingress) dashboard REMOVED 2026-09-06 (ADR-0040,
# supersedes Envoy Gateway/ADR-0008): grafana/dashboards/lab-envoy.json and the
# observability-alloy.yaml envoy_gateway_controller/envoy_proxy scrape blocks
# it depended on are both gone — Traefik replaced Envoy Gateway and does not
# expose Prometheus metrics by default (tracked as an explicit gap in ADR-0040).
