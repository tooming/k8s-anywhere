...
@test "allow-cert-manager-webhook-from-apiserver uses ipBlock for the kube-apiserver source" {
  run grep -q 'ipBlock:' "$CERT_MANAGER_NP/allow-cert-manager-webhook-from-apiserver.yaml"
  [ "$status" -eq 0 ]
}

@test "allow-cert-manager-webhook-from-apiserver uses Ingress policyType" {
  run grep -q 'Ingress' "$CERT_MANAGER_NP/allow-cert-manager-webhook-from-apiserver.yaml"
  [ "$status" -eq 0 ]
}
