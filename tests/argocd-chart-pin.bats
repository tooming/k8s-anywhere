...
@test "argocd chart_version default is pinned to 10.2.3 (RFC #785)" {
  run sed -n '/^variable "chart_version" {/,/^}/p' "$VARS"
  [ "$status" -eq 0 ]
  [[ "$output" == *'default     = "10.2.3"'* ]]
}

@test "argocd terragrunt.hcl inputs don't silently override the module default" {
  grep -q 'chart_version = "10.2.3"' "$LOCAL_TG"
  grep -q 'chart_version = "10.2.3"' "$ORACLE_TG"
}
