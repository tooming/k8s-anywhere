...
# host-based IngressRoutes declared in gitops/ (the source of truth — works with the
...
for h in $route_hosts; do
  grep -qx "$h" <<<"$panel_hosts" || bad "UI '$h' has an IngressRoute but is MISSING from the Lab UIs panel"
done
# advertised but not routed -> a stale row (GitLab/Grafana are excluded above)
for h in $panel_hosts; do
  grep -qx "$h" <<<"$route_hosts" || bad "Lab UIs panel lists '$h' but no IngressRoute declares it (stale row?)"
done

# Port check: host-based UIs must use the stable front-door port :8000. The front door
# routes to whichever cluster is active across a blue/green cutover; the per-cluster Traefik
...
[ "$drift" -eq 0 ] && printf '  %s✓%s Lab UIs panel and README.md Endpoints table both match the host-based IngressRoutes in gitops (and use the :8000 front door)\n' "$G" "$Z"
exit "$drift"
