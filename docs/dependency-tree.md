...
graph TD
  ...
  traefik -.->|"kiali.127.0.0.1.nip.io (on-demand)"| kiali

  %% --- Longhorn on-demand ---
  longhornmgr -.->|"manages volumes"| longhornui
  traefik -.->|"longhorn.127.0.0.1.nip.io (on-demand)"| longhornui
  ...
  traefik -.->|"tidb-demo.127.0.0.1.nip.io (on-demand)"| tidbdemo
...
| — | kiali *(on-demand)* | Service mesh observability UI (chart `kiali-server` from `https://kiali.org/helm-charts`, v2.31.0); anonymous auth; connects to Mimir Prometheus API; Traefik IngressRoute `kiali.127.0.0.1.nip.io` — use `make kiali-up` (requires `istio-up`) or `make mesh-up` |
| — | kiali-extras *(on-demand)* | Traefik IngressRoute for `kiali.127.0.0.1.nip.io`; paired with the kiali Application — use `make kiali-up` |
...
| — | longhorn-extras *(auto-synced, wave 0)* | Pre-creates the `longhorn-system` namespace with PSA `privileged` labels + Traefik IngressRoute for `longhorn.127.0.0.1.nip.io`; always-on so the PSA floor is present before `make longhorn-up` admits pods — the longhorn Application itself remains on-demand |
...
| Traefik → tidb-demo.127.0.0.1.nip.io *(on-demand)* | HTTPRoute | `gitops/tidb-demo/ingressroute.yaml` |
...
| Traefik → kiali.127.0.0.1.nip.io *(on-demand)* | HTTPRoute | `gitops/kiali/ingressroute.yaml` |
| istiod → Kiali *(on-demand)* | mesh config (xDS endpoint) | `gitops/platform/kiali.yaml` values |
| Traefik → longhorn.127.0.0.1.nip.io *(on-demand)* | HTTPRoute | `gitops/longhorn/ingressroute.yaml` |
