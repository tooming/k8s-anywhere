...
| 4 | ack-resources, data-demo, capstone-networkpolicy, data-networkpolicy, observability-networkpolicy, vault-networkpolicy, storage-networkpolicy | ACK `Bucket` CRs (need the controller); data-demo traffic generators (need rabbitmq + valkey); NetworkPolicy fan-out overlays (need namespaces created by waves 1–3) |
