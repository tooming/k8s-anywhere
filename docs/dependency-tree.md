...
| 1 | vault, external-secrets, garage, mimir, kube-state-metrics, moto, lab-gateway, kyverno | secret engine + ESO controller + storage + metrics store; shared Gateway (after Gateway API CRDs); Kyverno admission engine + CRDs |
...
| 4 | ack-resources, data-demo, kyverno-networkpolicy; *generated by `networkpolicy` AppSet:* ack-networkpolicy, argocd-networkpolicy, capstone-networkpolicy, data-networkpolicy, lab-gateway-networkpolicy, moto-networkpolicy, observability-networkpolicy, storage-networkpolicy, vault-networkpolicy | ACK `Bucket` CRs (need the controller); data-demo traffic generators (need rabbitmq + valkey); kyverno-networkpolicy default-deny overlay (kyverno ns); NetworkPolicy fan-out overlays generated by `networkpolicy` AppSet (need namespaces created by waves 1–3) |
