...
- **argocd namespace PSS Phase 1** (ADR-0017 §Staged rollout, RFC #205) — `gitops/argocd/namespace.yaml` adds `warn: restricted` + `warn-version: latest` + `audit: restricted` + `audit-version: latest` PSA labels to the `argocd` namespace. Phase 1 establishes the auditing floor: non-compliant pods surface in cluster events and audit logs without breaking ArgoCD admission. The `enforce` label is intentionally absent — that is Phase 2 (separate ROADMAP item, pending `infra/modules/argocd/values.yaml` securityContext overrides). Delivered by the auto-synced `argocd-extras` ArgoCD Application (`gitops/platform/argocd-extras.yaml`, sync-wave 0, `ServerSideApply=true`, `CreateNamespace=false`) which SSA-patches only the PSA label fields onto the Terraform-created namespace without claiming ownership of Terraform-managed fields.
