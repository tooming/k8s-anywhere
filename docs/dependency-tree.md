...
| 4 | ack-resources, data-demo, kyverno-networkpolicy, trivy-system-networkpolicy, argo-rollouts-networkpolicy, envoy-gateway-system-networkpolicy, velero-networkpolicy, kargo-networkpolicy, kargo-project-networkpolicy; *generated by `networkpolicy` AppSet:* ack-networkpolicy, argocd-networkpolicy, capstone-networkpolicy, data-networkpolicy, external-secrets-networkpolicy, lab-gateway-networkpolicy, kro-networkpolicy, moto-networkpolicy, observability-networkpolicy, storage-networkpolicy, tidb-networkpolicy, tidb-admin-networkpolicy, lab-demo-networkpolicy, vault-networkpolicy, inkless-networkpolicy, istio-system-networkpolicy, longhorn-networkpolicy, artifactory-networkpolicy, harbor-networkpolicy, node-exporter-networkpolicy | ACK `Bucket` CRs (need the controller); data-demo traffic generators (need rabbitmq + valkey); kyverno-networkpolicy default-deny overlay (kyverno ns); trivy-system-networkpolicy default-deny overlay (trivy-system ns); argo-rollouts-networkpolicy default-deny overlay (argo-rollouts ns); envoy-gateway-system-networkpolicy default-deny overlay (envoy-gateway-system ns); velero-networkpolicy default-deny overlay (velero ns — always-on; TCP 8085 metrics ingress, TCP 3900 Garage S3 egress, Kopia PV-read egress to backed-up namespaces); kargo-networkpolicy default-deny overlay (kargo ns — on-demand, pairs with kargo Application; TCP 80 Envoy ingress, TCP 9443 webhook callback ingress, TCP 80 ArgoCD egress, registry egress); kargo-project-networkpolicy default-deny overlay (capstone-pipeline ns — on-demand, pairs with kargo-project Application; kargo + argocd egress for promotion-step pods, ADR-0016 + ADR-0023); external-secrets-networkpolicy default-deny overlay (external-secrets ns); lab-demo-networkpolicy default-deny overlay (lab-demo ns — closes the ADR-0016 §4 always-on fan-out gap); NetworkPolicy fan-out overlays generated by `networkpolicy` AppSet (need namespaces created by waves 1–3); tidb/tidb-admin overlays are auto-synced ahead of on-demand `make tidb-up` so policies are in place when pods arrive; appset fan-out additions: kro-networkpolicy (kro ns), inkless-networkpolicy (inkless ns — on-demand), istio-system-networkpolicy (istio-system ns — on-demand), longhorn-networkpolicy (longhorn-system ns — on-demand), artifactory-networkpolicy (artifactory ns — on-demand), harbor-networkpolicy (harbor ns — on-demand), node-exporter-networkpolicy (node-exporter ns — always-on DaemonSet); *generated by `governance` AppSet:* one `<ns>-governance` Application per always-on namespace applying a `standard`-tier LimitRange container default (`argocd`, `capstone`, `kyverno`, `external-secrets`, `velero`, `argo-rollouts`, `trivy-system`, `moto`, `ack-system`, `kro`, `kargo`, `lab-demo`, `data`, `storage`, `vault`, `lab-gateway`, `kiali`, `harbor`, `envoy-gateway-system`, `node-exporter`) plus `observability-governance` at the `heavy` tier — RFC #294 fan-out complete (harbor LimitRange added per RFC #297 / ADR-0024; `envoy-gateway-system` and `node-exporter` LimitRange defaults added per ROADMAP `auto/governance-envoy-node-exporter`); on-demand heavy namespaces (`tidb`, `longhorn-system`, `istio-system`, `inkless`) are excluded as too variable for static defaults |
