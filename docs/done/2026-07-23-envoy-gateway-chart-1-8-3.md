...
(CHARTER **Core Values** §"Everything as code" + general hardening; RFC #671 — architect decision
...
- `gitops/platform/envoy-gateway.yaml`: `targetRevision` bumped `v1.8.2` → `v1.8.3`.
- `docs/decisions/adr-0008-envoy-gateway-not-traefik.md`: new `## Re-evaluation log`
  entry (2026-07-23) recording the bump, the GitHub release + Docker Hub
  verification evidence, and a new flip condition (`> v1.8.3`); the `## Files`
  table's inline pin note updated to `v1.8.3`.
- New `tests/envoy-gateway.bats`: asserts the Application exists, is
  auto-synced, sources the `docker.io/envoyproxy` chart, pins `v1.8.3`, and
  does **not** pin the superseded `v1.8.2` — a recurrence guard.
- `docs/dependency-tree.md` not touched — it doesn't reference the pinned
  chart version explicitly for Envoy Gateway (verified via grep before
  starting), so there was nothing to update.

**ADR-0004 caveat:** this is a remote, clusterless session — it cannot verify
Envoy Gateway actually starts cleanly post-bump on a live cluster. Rollback
path: revert `targetRevision` to `v1.8.2`, push; ArgoCD self-heals within its
sync interval. Envoy Gateway's control plane is stateless, so a revert
