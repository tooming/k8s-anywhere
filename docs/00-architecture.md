...
12. **Cloud-agnostic infrastructure design** — read [`infra/live/README.md`](../infra/live/README.md): the `argocd`/`gitlab` Terragrunt units depend only on the `cluster` unit's `kube_context`/`cluster_name`/`api_endpoint` outputs, never on which backend produced them. `local/` (k3d) is the only backend built today, but the contract is what would let steps 1–11 above run identically on a second backend without forking anything in `gitops/` — see [ADR-0026](decisions/adr-0026-cloud-agnostic-infrastructure.md) for the design decision, and [ADR-0028](decisions/adr-0028-oracle-cloud-backend-rejected.md) for why the first concrete attempt at a second backend was reverted. The lesson: portability is a property of *where the Terraform bootstrap seam sits*, not something bolted on afterward — GitOps state in `gitops/` never needs to know or care where the cluster runs.
