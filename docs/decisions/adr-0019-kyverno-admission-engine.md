...
- **Namespace:** `kyverno` (new namespace; PSA label `baseline` — Kyverno
  controllers run as non-root but the admission/cleanup controllers mount
  webhook TLS material that today requires `fsGroup` outside the `restricted`
  carve-out set; revisit when the chart documents `restricted` compatibility).
...
|-----------|-------------|--------|
| `kyverno` | `baseline` | Controllers run as non-root but mount webhook TLS via `fsGroup`; chart does not yet document `restricted` compatibility. Re-evaluate per chart upgrade. |
