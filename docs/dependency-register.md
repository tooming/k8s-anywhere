...
| Grafana | always-on-core (observability stack) | grafana.com, github.com/grafana/grafana | [ADR-0006](decisions/adr-0006-grafana-native-git-sync.md) | 2026-09-04 (ADR-0006's Re-evaluation log gained a newer entry for a sibling component — Tempo's own currency re-check, see that row below — with no new finding for Grafana itself; still current at `13.0.8`/chart `12.10.4`. Prior entry: 2026-09-03 security bump, `v13.0.8` fixes 3 named CVEs) |
...
| Aiven Inkless | heavy-on-demand (`make inkless-up`/`inkless-down`) | github.com/aiven/inkless | [ADR-0015](decisions/adr-0015-inkless-diskless-kafka.md) | 2026-09-03 (coverage sweep: documented the `danielqsj/kafka-exporter:v1.9.0` observability sidecar, previously unmentioned in the ADR — confirmed still current, zero GHSA advisories. Prior same-day entry: broker image bumped `4.2.1-0.46` → `4.2.1-0.47`, no CVE — real bug fixes; DB migration caveat flagged, tracked via a `[Manual step]` issue — see ADR-0015's own Re-evaluation log) |
