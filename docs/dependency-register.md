...
|---|---|---|---|---|
| Terraform / Terragrunt | always-on-core (day-0 bootstrap only, ADR-0001) | terraform.io, terragrunt.gruntwork.io | [ADR-0001](decisions/adr-0001-gitops-over-terraform-helm.md) | 2026-09-06 (JANITOR-fallback currency sweep — first review this row had ever recorded; ADR-0001 has no dedicated Re-evaluation log of its own, same shape as this run's ArgoCD sweep, so the result is recorded here directly. Terraform bumped `1.15.9`→`1.16.1`: routine currency, hashicorp/terraform's only published GitHub security advisory — GHSA-4rvg-555h-r626, an Azure-backend cleartext-state issue from 2019 — doesn't apply, this repo has never used an Azure backend (ADR-0007, Garage S3). Terragrunt bumped `v1.1.3`→`v1.1.4`: real security hardening, tightened generated-file permissions (0600/0700) + reduced credential duplication, no CVE filed. Both checked against their real release notes directly for breaking changes — none apply to this repo's actual usage. Full writeup: [docs/done/2026-09-06-terraform-terragrunt-currency-bump.md](done/2026-09-06-terraform-terragrunt-currency-bump.md)) |
...
| Argo Rollouts | always-on-next-wave (Objective O1) | github.com/argoproj/argo-rollouts | [ADR-0020](decisions/adr-0020-argo-rollouts-progressive-delivery.md) | 2026-09-07 (architect audit: dashboard-only unauthenticated-mutation CVE, GHSA-366v-5xmx-36vh/CVE-2026-82277, Critical, filed against appVersion `v1.10.0` — this lab's exact pin. No patched version exists yet; converted to [RFC #1479](https://github.com/tooming/k8s-anywhere/issues/1479) (Traefik basicAuth compensating control) rather than kept silently. Controller/CRDs unaffected — chart pin itself not bumped. See ADR-0020's own Re-evaluation log. Prior entry: 2026-09-01, chart `2.41.1` → `2.43.0` routine currency bump) |
...
| Harbor (supersedes Artifactory, ADR-0011) | heavy-on-demand (`make harbor-up`/`harbor-down`) | github.com/goharbor/harbor-helm | [ADR-0024](decisions/adr-0024-harbor-not-artifactory.md) | 2026-09-07 (**correction**: the 2026-08-20 entry's `GHSA-56j8-6qr5-cg75` for CVE-2026-4404, described there as "Low... disputed... not a legitimate vulnerability", does not exist — confirmed via a direct fetch, HTTP 404 — a fabricated/hallucinated GHSA ID and mischaracterized severity from that sweep (ADR-0004), which also never cross-referenced this ADR's own 2026-07-28 Re-evaluation log entry auditing the *same* CVE number correctly. The real advisory is **GHSA-hj7x-hmf2-hc2p**, **Critical** (CVSS 9.4, CWE-798/CWE-1393, NOT disputed): Harbor ships hard-coded default credentials (`admin`/`Harbor12345`) for `<= 2.15.0` — matching ADR-0024's own already-correct 2026-07-28 audit (`<=2.15.0`, fixed `2.15.1`+, kept — this lab's `appVersion: 2.15.2` is past the fixed floor, and independently never relies on the default since `gitops/secrets/harbor-admin-externalsecret.yaml` overrides it with a Vault-generated password, ADR-0036). GHSA-prh4-vhfh-24mj (Moderate, LDAP/OIDC secret redaction) reconfirmed unchanged: affects `>2.13.0,<2.14.3`, patched `2.13.5`/`2.14.3`/`2.15.0`, current appVersion past the floor. Chart `1.19.2` reconfirmed still the newest tag (2026-09-07, GitHub's own releases page) — no currency gap. Prior entry: 2026-08-20) |
...
| Forgejo | always-on-core (self-hosted git source + CI runner, host-level Docker Compose, outside the cluster) — **the live, running component as of 2026-08-17** (PR #1205's accelerated cutover); supersedes GitLab, whose `docker-compose.yml`/`infra/modules/gitlab-config` are still in the repo, stopped but kept for rollback until ROADMAP's remaining migration items (script/Makefile rename, full decommission) land | code.forgejo.org/forgejo/forgejo, code.forgejo.org/forgejo/runner | [ADR-0035](decisions/adr-0035-forgejo-not-gitlab.md) (supersedes [ADR-0033](decisions/adr-0033-gitlab-git-source-and-ci.md)) | 2026-09-07 (coverage/hardening sweep: bumped `forgejo` `16.0.2`→`16.0.3` — real security fixes per upstream's own release notes, confirmed via a GitHub mirror (codeberg.org/forgejo.org both egress-blocked from this sandbox): PR-update access control, reusable-workflow `pull_request_target` expansion, and a collaboration-access-mode issue, plus dependency security patches. Bumped `forgejo-runner` `13.0.0`→`13.1.0`, routine currency, no named CVE. Neither pull verified from this sandbox (registry unreachable) — the live `make up`/`forgejo-up` path is the real verification, same standing caveat as the 2026-09-06 registry-switch finding. Prior entry: 2026-08-17, live cutover, PR #1205) |
