...
| ArgoCD | always-on-core | argoproj.github.io, github.com/argoproj/argo-cd | [ADR-0001](decisions/adr-0001-gitops-over-terraform-helm.md) | 2026-08-13 (Terraform-bootstrapped `argo-cd` chart bumped `10.3.2` → `10.3.3`, real appVersion move `v3.5.0` → `v3.5.1` with security fixes — secret mask spoofing, secret exposure in last-applied-configuration annotation) |
...
| Grafana | always-on-core (observability stack) | grafana.com, github.com/grafana/grafana | [ADR-0006](decisions/adr-0006-grafana-native-git-sync.md) | 2026-08-13 (Tempo image tag bumped `2.10.7`→`2.10.8`, real security fixes — Go 1.26.5 stdlib CVEs plus grpc/otel/x-net/x-text/compress `[security]` dependency bumps; Grafana `13.0.5` and Loki `3.7.6` not re-checked this pass) |
...
| Aiven Inkless | heavy-on-demand (`make inkless-up`/`inkless-down`) | github.com/aiven/inkless | [ADR-0015](decisions/adr-0015-inkless-diskless-kafka.md) | 2026-08-05 (`postgres` batch-coordinator image held at the `17.x` line, issue #1013) |
...
| Kargo | heavy-on-demand (`make kargo-up`/`kargo-down`) | github.com/akuity/kargo | [ADR-0023](decisions/adr-0023-kargo-promotion-pipeline.md) | 2026-08-11 (chart bumped `1.11.0` → `1.11.1`, PR #1101) |
...
| kube-state-metrics | always-on-core (observability — Kubernetes object-state exporter) | github.com/kubernetes/kube-state-metrics | [ADR-0034](decisions/adr-0034-lgtmp-observability-stack.md) | 2026-08-12 (chart bumped `8.2.0` → `8.3.0`, packaging-only release — `appVersion` unchanged) |
