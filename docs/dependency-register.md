...
| Kiali | heavy-on-demand (`make kiali-up`/`kiali-down`) | kiali.io, github.com/kiali/kiali | [ADR-0012](decisions/adr-0012-istio-ambient-not-sidecar.md) | 2026-09-01 (row date touched only because this ADR's Re-evaluation log gained an Istio-chart entry this date — checked, not assumed: `kiali/helm-charts`' real tags.atom feed now shows `v2.31.0` (2026-08-24), one release past this lab's `2.30.0` pin. Genuine currency gap, out of scope for this PR — flagged for a follow-up upgrade-drafter cycle) |
