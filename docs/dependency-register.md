...
| Grafana | always-on-core (observability stack) | grafana.com, github.com/grafana/grafana | [ADR-0006](decisions/adr-0006-grafana-native-git-sync.md) | 2026-09-03 (Grafana's own last real currency check is still 2026-08-19 — `v13.0.7`, `CVE-2026-17183` — unchanged this cycle; date bumped only because ADR-0006's own Re-evaluation log's newest entry as of 2026-09-03 is the Loki `3.7.6`→`3.7.7` bump below, and `dependency-register-check.sh`'s `###`-heading shape can't distinguish which shared-ADR component a heading is about, a documented limitation in the script's own header comment) |
