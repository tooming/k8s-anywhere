...
| ArgoCD | always-on-core | argoproj.github.io, github.com/argoproj/argo-cd | [ADR-0001](decisions/adr-0001-gitops-over-terraform-helm.md) | 2026-08-19 (GHSA sweep: 2 Critical — GHSA-3v3m-wc6v-x4x3 affects `3.2.0`-`3.3.8`/patched `3.3.9`,`3.2.11`; GHSA-786q-9hcg-v9ff affects `>=2.2.0-rc1`/patched `3.1.2`,`3.0.14` — and 1 High GHSA-h98r-wv3h-fr38 affects `<3.0.0`/patched `3.4.2` — current `v3.5.1` (chart `10.4.0`) past every floor and is the newest chart tag; no currency gap) |
...
| Kyverno | always-on-next-wave (Objective O1) | github.com/kyverno/kyverno | [ADR-0019](decisions/adr-0019-kyverno-admission-engine.md) | 2026-08-18 (`disallow-latest-tag`'s `inkless` carve-out flip condition met — Aiven Inkless shipped a stable numbered release line — exclusion removed, PR #1217, no flip condition remaining. Prior entry: 2026-08-06, the `argocd` carve-out's own flip condition met the same way, issue #999/PR #1037) |
| Argo Rollouts | always-on-next-wave (Objective O1) | github.com/argoproj/argo-rollouts | [ADR-0020](decisions/adr-0020-argo-rollouts-progressive-delivery.md) | 2026-08-19 (GHSA sweep: zero published security advisories exist for this repo at all) |
...
| Trivy Operator | always-on-next-wave (Objective O1) | github.com/aquasecurity/trivy-operator | [ADR-0022](decisions/adr-0022-trivy-operator-supply-chain.md) | 2026-08-19 (GHSA sweep: zero published security advisories exist for this repo at all — checked directly, `github.com/aquasecurity/trivy-operator/security/advisories` is empty; no CVE-driven action possible or needed) |
| Kargo | heavy-on-demand (`make kargo-up`/`kargo-down`) | github.com/akuity/kargo | [ADR-0023](decisions/adr-0023-kargo-promotion-pipeline.md) | 2026-08-19 (GHSA sweep: 1 Critical GHSA-7g9x-cp9g-92mr affects `>=1.7.0,<=1.9.2`/patched `1.9.3`; 2 Moderate (GHSA-xx8h-gw9m-m95p, GHSA-f72x-6fm6-94rq) affect `<1.11.0`/patched `1.9.10`,`1.10.9` (1.11.0+ unaffected); GHSA-wp4p-hr79-q4g8 Moderate patched `1.10.8`+"upcoming 1.11"; GHSA-j94x-8wcp-x7hm Moderate SSRF patched `1.9.5` — current pin `1.11.2` past every floor and is the newest tag; no currency gap) |
...
| KEDA | always-on-core | github.com/kedacore/keda | [ADR-0029](decisions/adr-0029-keda-event-driven-autoscaling.md) | 2026-08-19 (GHSA sweep: GHSA-c4p6-qg4m-9jmr High affects operator `≤2.17.2`,`≤2.18.2`/patched `2.17.3`,`2.18.3`,`≥2.19.0`; GHSA-6w3m-4hhp-775q Moderate affects `≤2.19.x`/patched `2.20` — current pin `2.20.2` past both floors and is the newest tag; no currency gap) |
